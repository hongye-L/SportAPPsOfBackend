...
public class ReplyOfTeachingDAO {
    ...
    public int addReply(replys replys){
        String sql="insert into sportapp.reply_teaching(reply_id,nickname,content) values(:reply_id,:nickname,:content)";
    ...
    public int updateReply(replys replys) {
        String sql = "update sportapp.reply_teaching set content=:comment,nickname=:name  where reply_id=:id";
        return new NamedParameterJdbcTemplate(jdbcTemplate).update(sql,new BeanPropertySqlParameterSource(replys));
    }
    ...
    public replys getreplys(String id){
        ...
        List<replys> list =jdbcTemplate.query(sql,new Object[]{id},new BeanPropertyRowMapper(comments.class));
        //判断list里面有没有东西
